...
def _back_to_xhtml(et):
    m = entry_stripper.match(ET.tostring(et, 'utf-8'))
    if m:
        return m.group(1)
    else:
...
def resize_images_to_fit_width(value, arg):
    max_width = int(arg)
    et = ET.fromstring('<entry>%s</entry>' % value.encode('utf8'))
...
def strip_p_ids(xhtml):
    et = ET.fromstring('<entry>%s</entry>' % xhtml)
...
def typography(xhtml):
    ...
    et = ET.fromstring(u'<entry>%s</entry>' % xhtml.encode('utf8'))
    do_typography(et)
    return _back_to_xhtml(et)
...
from django.utils.timesince import timesince # Reuse that logic
import datetime
